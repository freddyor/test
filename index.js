...
topicsButton.style.maxHeight = '28px';
topicsButton.style.boxSizing = 'border-box';
topicsButton.style.lineHeight = '28px';
topicsButton.style.paddingTop = '0';
topicsButton.style.paddingBottom = '0';
topicsButton.style.marginTop = '0';
topicsButton.style.marginBottom = '0';
topicsButton.style.background = '#e0e0e0';
topicsButton.style.color = '#111';
topicsButton.style.border = '2px solid #111';
topicsButton.style.borderRadius = '14px';
topicsButton.style.boxShadow = '0 2px 8px rgba(0,0,0,0.12)';
topicsButton.style.fontSize = '15px';
topicsButton.style.fontFamily = "'Poppins', sans-serif";
topicsButton.style.fontWeight = 'bold';
topicsButton.style.cursor = 'pointer';
topicsButton.style.padding = '0 15px';
topicsButton.style.display = 'flex';
topicsButton.style.alignItems = 'center';
topicsButton.style.justifyContent = 'center';
topicsButton.style.marginLeft = '12px';
topicsButton.style.transition = 'box-shadow 0.2s';

// Topics dropdown menu
const topicsDropdown = document.createElement('div');
topicsDropdown.id = 'topics-dropdown';
topicsDropdown.style.position = 'fixed';
topicsDropdown.style.top = '50px';
topicsDropdown.style.left = '10px';
topicsDropdown.style.background = '#e0e0e0';
topicsDropdown.style.border = '2px solid #111';
topicsDropdown.style.borderRadius = '14px';
topicsDropdown.style.boxShadow = '0 2px 8px rgba(0,0,0,0.12)';
topicsDropdown.style.zIndex = '9999';
topicsDropdown.style.display = 'none';
topicsDropdown.style.flexDirection = 'column';
topicsDropdown.style.minWidth = '150px';
topicsDropdown.style.padding = '8px';
topicsDropdown.style.gap = '4px';

function createTopicButton(topic) {
  const btn = document.createElement('button');
  btn.textContent = topic.charAt(0).toUpperCase() + topic.slice(1);
  btn.style.padding = '8px 12px';
  btn.style.background = selectedTopic === topic ? '#9b4dca' : '#fff';
  btn.style.color = selectedTopic === topic ? '#fff' : '#111';
  ...
  btn.onclick = () => filterMarkersByTopic(topic);
  return btn;
}
...
function updateTopicsDropdown() {
  ...
  allBtn.style.background = selectedTopic === null ? '#9b4dca' : '#fff';
  allBtn.style.color = selectedTopic === null ? '#fff' : '#111';
  allBtn.style.border = '1px solid #111';
  allBtn.style.borderRadius = '8px';
  allBtn.style.cursor = 'pointer';
  allBtn.style.fontFamily = "'Poppins', sans-serif";
  allBtn.style.fontWeight = 'bold';
  allBtn.style.fontSize = '14px';
  allBtn.style.transition = 'all 0.2s';
  
  allBtn.onclick = () => filterMarkersByTopic(null);
  topicsDropdown.appendChild(allBtn);
  
  // Get unique topics from buildings
  const topics = [...new Set(buildings.map(b => b.topic).filter(t => t))];
  topics.forEach(topic => {
    topicsDropdown.appendChild(createTopicButton(topic));
  });
}

topicsButton.onclick = () => {
  updateTopicsDropdown();
  topicsDropdown.style.display = topicsDropdown.style.display === 'none' ? 'flex' : 'none';
};
...
function filterMarkersByTopic(topic) {
  ...
  document.querySelectorAll('.building-marker').forEach((marker) => {
    ...
    if (selectedTopic === null || (building && building.topic === selectedTopic)) {
...
progressBarWrapper.appendChild(topicsButton); // ADD TOPICS BUTTON
document.body.appendChild(progressBarWrapper);
document.body.appendChild(topicsDropdown); 
